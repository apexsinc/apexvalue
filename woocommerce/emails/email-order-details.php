...
/**
 * APEX VALUE OVERRIDE (v1.11.7):
 * This site sells via quotation requests only (quotes-for-woocommerce,
 * global quote mode) and never exposes prices. Quote orders carry the
 * `_quote_status` meta — for them the Price column and the Subtotal/Total
 * rows are omitted entirely. Non-quote orders (if ever enabled) keep the
 * original WooCommerce layout.
 */
$is_apex_quote_order = $order instanceof WC_Order && (bool) $order->get_meta( '_quote_status', true );
...
<div style="margin-bottom: <?php echo $email_improvements_enabled ? '24px' : '40px'; ?>;">
	...
	<?php endif; ?>
	<?php if ( $is_apex_quote_order && $order->get_customer_note() ) : ?>
